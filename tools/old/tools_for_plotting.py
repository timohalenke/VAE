...
class Train_Insights():
    def __init__(self, netG, real_spectra, columns, loss_metric_manager, n_to_generate = 'auto', scaler=None, epoch=None, vec=None):
    ...
    def Include_Distributions(self, save_results=False, destination_folder='', make_kde_plot=False, kde_spectra_cap=500):
        ...
        for i in range(n_spectra_to_plot):
            ax1.plot(x, self.generated_spectra[i], color='r', label=f'simulated data' if i == 0 else "", lw=1)
            # ax1.plot(x, self.real_spectra[i], color='b', label='real data' if i == 0 else "", lw=1)
        ...
        # ax2 = brokenaxes(xlims=((int(1000.8161432865553), int(1798.4815784117218)), (int(2800.3801925785974), int(2998.8331873078055))), hspace=.02)
        ...
        # ax2.set_ylabel('Absorbance [a.u.]', fontsize=14)
        ...
        # gs = GridSpec(1, 2, width_ratios=[4, 1])  # Setze die Breitenverhältnisse

        # # Erstellen der Subplots mit unterschiedlichen Breiten
        # ax2_1 = fig2.add_subplot(gs[0])
        # ax2_2 = fig2.add_subplot(gs[1])
        
        

        # # Plot: Mittelwert der "generated" Spektren (GAN data)
        # ax2_1.plot(x, np.mean(self.generated_spectra, axis=0), color='r', label='simulated data', lw=0.5)
        # ax2_2.plot(x, np.mean(self.generated_spectra, axis=0), color='r', label='simulated data', lw=0.5)

        # # Plot: Mittelwert der "real" Spektren (real data)
        # ax2_1.plot(x, np.mean(self.real_spectra, axis=0), color='b', label='real data', lw=0.5)
        # ax2_2.plot(x, np.mean(self.real_spectra, axis=0), color='b', label='real data', lw=0.5)

        # # Plot der einzelnen Spektren
        # for i in range(n_spectra_to_plot):
        #     # "Generated" Spektren
        #     ax2_1.plot(x, self.generated_spectra[i], color='r', lw=0.5, alpha=alpha_per_spectrum)
        #     ax2_2.plot(x, self.generated_spectra[i], color='r', lw=0.5, alpha=alpha_per_spectrum)

        #     # "Real" Spektren
        #     ax2_1.plot(x, self.real_spectra[i], color='b', lw=0.5, alpha=alpha_per_spectrum)
        #     ax2_2.plot(x, self.real_spectra[i], color='b', lw=0.5, alpha=alpha_per_spectrum)

        # # Setze die x-Achsen-Limits für beide Subplots
        # ax2_1.set_xlim(1000.8161432865553, 1798.4815784117218)
        # ax2_2.set_xlim(2800.3801925785974, 2998.8331873078055)
        
        # ax2_1.set_xticks(np.arange(1000, 1800, 100))
        # ax2_2.set_xticks(np.arange(2800, 3000, 100))

        # # Spines anpassen, um den "gebrochenen" Achsen-Effekt zu erzeugen
        ...
        # ax2_1.tick_params(labelright=False)
        # ax2_2.tick_params(right=False, labelright=False)
        ...
        # ax2_2.plot([0, 0], [1, 0], transform=ax2_2.transAxes, **kwargs)

        # # # Achsenbeschriftungen

        # ax2_1.set_ylabel('Absorbance [a.u.]')
        # ax2_1.set_xlabel('Wavenumber [cm^-1]')
        # ax2_2.legend(loc='upper right')
        # ax2_1.set_title('Spectral Profiles Real vs Simulated')
        ...
        # ax4_2 = fig4.add_subplot(gs[1])

        # # Plot der Daten in beiden Subplots
        # ax4_1.plot(x, diff_fp, color='dimgrey', label='diff fp')
        # ax4_2.plot(x, diff_fp, color='dimgrey', label='diff fp')

        # # Füllbereich
        # ax4_1.fill_between(x, diff_fp - std_diff_fp, diff_fp + std_diff_fp, color='gray', alpha=0.2, label='std diff fp')
        # ax4_2.fill_between(x, diff_fp - std_diff_fp, diff_fp + std_diff_fp, color='gray', alpha=0.2, label='std diff fp')

        # # Setze die x-Achsen-Limits für beide Subplots
        # ax4_1.set_xlim(1000.8161432865553, 1798.4815784117218)
        # ax4_2.set_xlim(2800.3801925785974, 2998.8331873078055)

        # # Manuelle Anpassung der x-Ticks
        # ax4_1.set_xticks(np.arange(1000, 1800, 100))
        # ax4_2.set_xticks(np.arange(2800, 3000, 100))
        ...
        # # Ticks anpassen
        # ax4_1.yaxis.tick_left()
        # ax4_2.yaxis.tick_right()
        # ax4_1.tick_params(labelright=False)
        # ax4_2.tick_params(right=False, labelright=False)
        
        # # Marker für den Achsenbruch anpassen
        # d = .5  # proportion of vertical to horizontal extent of the slanted line
        # kwargs = dict(marker=[(-1, -d), (1, d)], markersize=12,
        #             linestyle="none", color='k', mec='k', mew=1, clip_on=False)
        # ax4_1.plot([1, 1], [0, 1], transform=ax4_1.transAxes, **kwargs)
        # ax4_2.plot([0, 0], [1, 0], transform=ax4_2.transAxes, **kwargs)   

        # # # Achsenbeschriftungen
        
        # ax4_1.set_xlabel('Wavenumber [cm^-1]')
        # ax4_1.set_ylabel('Difference in Mean')
        # ax4_1.set_ylim([-0.005, 0.005])
        # ax4_2.set_ylim([-0.005, 0.005])
        # ax4_2.legend(loc='upper right')
        # ax4_1.set_title('Differential Fingerprint Real vs Simulated')
        ...
        #     ax6.plot(generated_data_ranges[0], generated_densities(generated_data_ranges[0]), alpha=0.3, color='r', label='simulated data')
        #     ax6.legend()
        ...
        if save_results == True:
            plt.savefig(f'{destination_folder}/Training_results_1_epoch_{self.epoch}.png', dpi=1000)
        plt.show()
    ...
    def Make_ROC(self, make_plot=False, save_plot=False, destination_folder='', spectra_cap = 1000):
        ...
        if make_plot == True:
            ...
            plt.tight_layout()
            if save_plot == True:
                plt.savefig(f'{destination_folder}/Training_results_3_epoch_{self.epoch}.png', dpi=1000)
            plt.show()
    ...
    def Include_Losses_and_Metrics(self, save_results=False, destination_folder=''):
        ...
        plt.tight_layout()
        if save_results:
            plt.savefig(f'{destination_folder}/Training_results_2_epoch_{self.epoch}.png', dpi=1000)
        plt.show()
